...
export const MovieSearchDetail = () => {
    const {id} =useParams();
  ...
  return (
     ...
     {movie && (
      <div className="max-w-md mx-auto bg-white dark:bg-zinc-900 rounded-xl shadow-lg overflow-hidden transform hover:scale-[1.02] transition duration-300">
       <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} />

      <div className="p-6 space-y-3">
        <h2 className="text-2xl font-bold text-zinc-800 dark:text-white">{movie.title}</h2>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">{movie.releaseDate} • {movie.genre} • {movie.runtime} min</p>

        <p className="text-zinc-700 dark:text-zinc-300 text-sm line-clamp-4">{movie.overview}</p>
